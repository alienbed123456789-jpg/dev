...
function Register() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            const errorMsg = typeof data === 'string' ? data : (data?.message || t('register_error'));
            setError(errorMsg);
        } finally {
            setLoading(false);
        }
        ...
        <div style={styles.container}>
            ...
            <div style={styles.card}>
                ...
                <form onSubmit={handleRegister} style={styles.form}>
                    ...
                    <button type="submit" disabled={loading} style={{...styles.button, background: '#48bb78'}}>
                        {loading ? '...' : t('register')}
                    </button>
                </form>
                ...
                <p style={styles.footer}>
                    {t('already_have_account')} <span onClick={() => navigate('/login')} style={styles.link}>{t('login')}</span>
                </p>
            </div>
        </div>
    );
...
const styles = {
    ...
    button: { padding: '15px', color: 'white', border: 'none', borderRadius: '15px', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold', transition: 'all 0.2s' },
    error: { color: '#fc8181', fontSize: '14px', margin: 0, textAlign: 'center' },
